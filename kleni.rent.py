...
def main():
    ...
    if συνολικές_κανονικές_μέρες > 0:
        απαραίτητη_τιμή_κανονικές = (
            (συνολικό_κόστος - εισόδημα_γιορτές - εισόδημα_καλοκαίρι) 
            / συνολικές_κανονικές_μέρες
        )
    ...
    st.write("""
        Παρακάτω βλέπεις ποια πρέπει να είναι η τιμή ενοικίου για τις κανονικές περιόδους,
        ώστε η Απόδοση (ROI) να είναι 0 (δηλαδή το συνολικό εισόδημα να καλύπτει
        ακριβώς το αρχικό σου budget).
    """)
    ...
    with col3:
        ...
        if γιορτές["Πάσχα"]["μέρες"] > 0:
            st.write(
                f"**Απαιτούμενο Εισόδημα (ανά έτος):** "
                f"€{γιορτές['Πάσχα']['μέρες'] * γιορτές['Πάσχα']['τιμή']:.2f}"
            )
            st.write(
                f"**Τρέχουσα Τιμή Ενοικίου:** "
                f"€{γιορτές['Πάσχα']['τιμή']:.2f}/ημέρα"
            )
        else:
    ...
    with col4:
        ...
        if συνολικές_καλοκαιρινές_μέρες > 0:
            st.write(
                f"**Απαιτούμενο Εισόδημα (ανά έτος):** €{εισόδημα_καλοκαίρι:.2f}"
            )
            st.write(
                f"**Τρέχουσα Τιμή Ενοικίου:** "
                f"€{τιμή_καλοκαίρι:.2f}/ημέρα"
            )
        else:
            st.write("Δεν υπάρχει ενοικίαση το καλοκαίρι.")

    with col5:
        st.subheader("Κανονικές Περίοδοι")
        if συνολικές_κανονικές_μέρες > 0:
            st.write(
                f"**Απαιτούμενο Εισόδημα για ROI=0 (ανά έτος):** "
                f"€{(συνολικό_κόστος - εισόδημα_γιορτές - εισόδημα_καλοκαίρι):.2f}"
            )
            st.write(
