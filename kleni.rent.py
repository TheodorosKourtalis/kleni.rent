...
# -------- Υπολογισμός Απαιτούμενων Τιμών Ενοικίασης για ROI=0 -------- #
...
# -- Σημείωση για το εισόδημα που χρειάζεται για ROI=0 στις Κανονικές Περίοδους --
# Για τις Κανονικές Περίοδους (ή όποια άλλη περίοδο θες), μπορείς να υπολογίζεις ξεχωριστά
# το "Απαιτούμενο Εισόδημα για ROI=0 (ανά έτος)" αν θέλεις να δείξεις τη διαφορά
# (συνολικό κόστος - ήδη καλυμμένο από άλλες περιόδους).
...
for chunk in period_chunks:
    ...
    for idx, (period_name, info) in enumerate(chunk):
        with cols[idx]:
            ...
            if days > 0:
                # -- Απαιτούμενο Εισόδημα (ανά έτος) = days * price (ή κάποια άλλη λογική) --
